...
const routes = [
  ...
  },
  {
    ...
    requiredMarkers: [
      ...
      { label: 'source refresh-needed state', text: 'Activity needs a fresh source check. Refresh Activity or open Settings Feature Wiring.' },
  ...
  },
  {
    ...
    requiredMarkers: [
      ...
      { label: 'calendar editor move guard', text: 'Existing events keep their current calendar in this editor' },
      { label: 'calendar draft binding', text: 'bind:value={eventDraft.calendarId}' },
      { label: 'calendar save refresh target', text: 'selectedCalendarId = savedDraft.calendarId' },
  ...
  },
  {
    ...
    safeAction: 'Open Productivity if the callback was opened manually or popup handoff does not complete automatically.',
    sampleInput: 'Open the callback without code/state to verify it stays on a Google OAuth recovery state instead of swapping route content.',
    expectedResult: 'OAuth completion posts to the opener or redirects back to the original hub; manual no-code visits stay on the callback with an actionable Productivity link.',
    reloadProof: 'Reloading the callback without code/state should still show the Google OAuth recovery page and link back to Productivity.',
    safeActionLabels: ['Open Productivity'],
    expectedBlockedState: 'Missing code/state stays on the callback when opened manually; OAuth errors or popup failures return to Productivity with an actionable message.',
    ...
    requiredMarkers: [
      ...
      { label: 'fallback link title', text: 'Open Productivity with the Google OAuth result message.' },
      ...
      { label: 'manual callback result', text: 'showManualResult(result)' },
  ...
  },
  {
    ...
    requiredMarkers: [
      ...
      { label: 'offline reports empty state', text: 'Saved reports will reload after the AI OS service card reconnects' },
      ...
      { label: 'offline monitor empty state', text: 'Saved topic monitors will reload after the AI OS service card reconnects' },
      { label: 'source library gating', text: 'sourceLibrarySearchDisabled' },
      { label: 'offline source empty state', text: 'Archived sources will reload after the AI OS service card reconnects' },
  ...
  },
  {
    ...
    requiredMarkers: [
      ...
      { label: 'macro editor input gate', text: 'macroEditorTextareaTitle' },
...
function liveHydrationState(row) {
  ...
  return 'static shell only; run qa:hub:hydrated for browser controls';
}
...
function printChecklist(rows, liveRows, baseUrl) {
  ...
  console.log('For hydrated browser clicks and persistence checks, run pnpm qa:hub:hydrated; pnpm qa:hub:usability runs static, hosted, and hydrated passes together.');
  console.log('');
  for (const row of rows) {
    const live = liveRows.get(row.id);
