...
const BLACK_HOLE_EXPAND_START = 0.52;
const BLACK_HOLE_EXPAND_END = 0.80;
...
const ABILITIES = (() => {
  ...
  add('rg_trapmaster', { cls: 'rogue', branch: 'bladeslinger', tier: 4, slot: 'passive', name: 'Bladecaller', desc: 'Keystone: loose knives tug toward you, knife spreads throw one more blade, and Bladecall adds extra seeking knives.', key: true, tags: ['Projectile', 'Pull'] });
...
const LAB_BUILDS = {
  ...
  rogue: [
    ...
    { id: 'bladecaller', name: 'Bladecaller', note: 'Knife control subclass: throw knives, recall loose blades, and use Q to call a full spectral blade storm back through targets.', loadout: { attack: 'rg_throw', secondary: 'rg_bladecall', shift: 'rg_slide', e: 'rg_tripwire', q: 'rg_bladecall_q', passive: 'rg_trapmaster' } },
...
const ROGUE_BACKFLIP_CLIP = {
  curl: [[0, 0.05], [0.12, 0.34], [0.28, 1.08], [0.48, 1.22], [0.67, 0.78], [0.84, 0.24], [1, 0]],
  tuck: [[0, 0], [0.16, 0.24], [0.30, 1.18], [0.52, 1.28], [0.66, 0.92], [0.82, 0.20], [1, 0]],
  open: [[0, 0], [0.58, 0], [0.70, 0.42], [0.86, 0.92], [1, 1]],
  lean: [[0, -0.10], [0.12, -0.44], [0.28, -0.80], [0.46, -0.50], [0.62, 0.12], [0.78, 0.28], [1, 0.03]],
  hipLift: [[0, 0], [0.16, -8], [0.36, -14], [0.60, -10], [0.84, -3], [1, 0]],
  headBack: [[0, 0], [0.14, 10], [0.28, 16], [0.46, 12], [0.62, -3], [0.80, -6], [1, 0]],
  headUp: [[0, 0], [0.15, -7], [0.34, -15], [0.52, -9], [0.70, -2], [1, 0]],
  footForward: [[0, 8], [0.16, 26], [0.30, 38], [0.50, 28], [0.72, 16], [1, 5]],
  footSpread: [[0, 10], [0.22, 5], [0.42, 2], [0.64, 10], [0.86, 18], [1, 9]],
  armForward: [[0, -8], [0.14, 14], [0.30, 28], [0.52, 21], [0.72, 9], [1, 0]],
  armDown: [[0, 20], [0.14, -4], [0.30, 24], [0.52, 29], [0.72, 10], [1, 21]],
};
...
PUBLIC.start = function (root, api) {
  ...
  style.textContent = `
    ...
    .sr-pose-open{border:1px solid rgba(156,255,94,.46);background:rgba(156,255,94,.12);color:#efffe7}
  ...
  function rogueFlipConfig(act) {
    ...
    return {
      dur: advanced ? 735 : tucked ? 700 : 675,
      vx: advanced ? 2.05 : tucked ? 1.92 : 1.78,
      burst: advanced ? 15 : tucked ? 13 : 12,
      tuck: advanced ? 1.34 : tucked ? 1.23 : 1.12,
      curl: advanced ? 1.28 : tucked ? 1.17 : 1.06,
  ...
  function useBladecallUltimate(ang) {
    if (!startVisualAttack('throw', ang, { kind: 'bladecallUltimate' })) return false;
    const cx = player.x;
    const cy = player.y - 62;
    const loose = droppedKnives && droppedKnives.length ? droppedKnives.splice(0, droppedKnives.length) : [];
    const bonus = Math.min(8, loose.length * 2) + (hasPassive('rg_trapmaster') ? 4 : 0);
    const total = Math.min(30, 16 + bonus);
    for (let i = 0; i < total; i++) {
      const fanT = total <= 1 ? 0.5 : i / (total - 1);
      const ring = i % 4;
      const side = i % 2 === 0 ? 1 : -1;
      const orbit = ang + Math.PI + fanT * Math.PI * 2 + side * rand(0.04, 0.16);
      const rx = 46 + ring * 20 + rand(-6, 9);
      const ry = 26 + ring * 13 + rand(-5, 8);
      const x = cx + Math.cos(orbit) * rx;
      const y = cy + Math.sin(orbit) * ry;
      const recalled = i < loose.length ? loose[i] : null;
      if (recalled) {
        spawnBladeRecallTrail(recalled.x, recalled.y, x, y, { life: 520, phase: i * 0.42, accent: '#f1ffe8' });
        for (let s = 1; s <= 5; s++) {
          const u = s / 6;
          particles.push({
            x: lerp(recalled.x, x, u) + rand(-5, 5),
            y: lerp(recalled.y, y, u) + rand(-5, 5),
            vx: (x - recalled.x) * 0.003 + rand(-0.18, 0.18),
            vy: (y - recalled.y) * 0.003 + rand(-0.18, 0.18),
            life: rand(220, 420),
            max: 420,
            color: Math.random() < 0.46 ? '#ffffff' : cls.color,
            r: rand(1.0, 2.8),
          });
        }
      } else if (i < 22) {
        spawnBladeRecallTrail(cx + rand(-12, 12), cy + rand(-10, 10), x, y, { life: 360, phase: i * 0.35, accent: cls.color });
      }
      const arcOut = (fanT - 0.5) * 1.18 + side * rand(0.28, 0.58);
      spawnDagger(ang + arcOut, {
        x, y,
        speed: recalled ? rand(1.9, 3.4) : rand(0.8, 2.2),
        hit: recalled ? 12.5 : 10.2,
        homing: true,
        summoned: true,
        noDrop: true,
        arm: 420 + i * 24,
        phase: orbit,
        curve: side * rand(9.0, 17.0) + (fanT - 0.5) * 9.0,
        curveLife: rand(620, 960),
        stagger: recalled ? 840 : 660,
        seekSpeed: recalled ? 29 : 26,
        seekRange: 940,
        life: 3300 + i * 22,
        color: i % 4 === 0 ? '#ffffff' : '#f1ffe8',
        quiet: true,
      });
      particles.push({
        x, y,
        vx: Math.cos(orbit) * rand(0.2, 0.9),
        vy: Math.sin(orbit) * rand(0.18, 0.72) - 0.10,
        life: rand(300, 620),
        max: 620,
        color: Math.random() < 0.42 ? '#ffffff' : cls.color,
        r: rand(1.1, 3.0),
      });
    }
    if (loose.length && player.knifeAmmo < ROGUE_MAX_KNIVES) {
      player.knifeAmmo = Math.min(ROGUE_MAX_KNIVES, player.knifeAmmo + Math.min(loose.length, 3));
      player.knifeRegen = 0;
    }
    burst(cx, cy, '#ffffff', 18, 3.8);
    burst(cx, cy, cls.color, 46 + loose.length * 3, 5.6);
    spawnShockwaveRing(cx, cy, 196, cls.color, { life: 620, width: 5.4, fill: 0.07, rough: 0.095 });
    addShake(4.2 + Math.min(3, loose.length * 0.35), 190);
    syncHud();
    return true;
  }
  ...
  function useMageSingularity() {
    ...
    const ang = Math.atan2(p.y - (player.y - 76), p.x - player.x);
    const source = staffTipOrigin(ang, { type: 'arcaneBloom', t: 0.42, tipPad: 4 });
    spawnBlackHole(p.x, p.y, player.team, gravityAccent(), { r: 296, life: 3800, pullPower: 1.56, sourceX: source.x, sourceY: source.y });
    return true;
  }
  ...
  function makeSpiritFollower(x, y, opts) {
    opts = opts || {};
    return {
      x, y,
      vx: 0, vy: 0,
  ...
  function ensureSpiritFollowers(owner, origin) {
    if (!owner) return [];
    if (!owner.spiritFollowers) owner.spiritFollowers = [];
    const wanted = clamp(Math.floor(owner.spiritCharges || 0), 0, 6);
    while (owner.spiritFollowers.length > wanted) owner.spiritFollowers.pop();
    while (owner.spiritFollowers.length < wanted) {
      const ox = origin && origin.x != null ? origin.x : owner.x - (owner.facing || 1) * rand(18, 52);
      const oy = origin && origin.y != null ? origin.y : owner.y - rand(54, 94);
      owner.spiritFollowers.push(makeSpiritFollower(ox, oy, origin || {}));
    }
    return owner.spiritFollowers;
  }
  function addSpiritFollower(owner, x, y, opts) {
    if (!owner) return null;
    opts = opts || {};
    if (!owner.spiritFollowers) owner.spiritFollowers = [];
    const wanted = clamp(Math.floor(owner.spiritCharges || 0), 0, 6);
    if (owner.spiritFollowers.length >= wanted) return owner.spiritFollowers[owner.spiritFollowers.length - 1] || null;
    const f = makeSpiritFollower(x, y, opts);
    owner.spiritFollowers.push(f);
    return f;
  }
  function spendSpiritFollower(owner, tx, ty, opts) {
    if (!owner) return null;
    opts = opts || {};
    ensureSpiritFollowers(owner);
    owner.spiritCharges = Math.max(0, (owner.spiritCharges || 0) - 1);
    const f = owner.spiritFollowers && owner.spiritFollowers.length ? owner.spiritFollowers.shift() : null;
    if (f) {
      emitSoulWisp(f.x, f.y, tx == null ? owner.x : tx, ty == null ? owner.y - 58 : ty, {
        count: opts.count || 12,
        color: f.color || opts.color || SPIRIT_COLORS.ghost,
        lifeMin: opts.lifeMin || 240,
        lifeMax: opts.lifeMax || 660,
      });
      for (let i = 0; i < 4; i++) soulParticle(f.x + rand(-4, 4), f.y + rand(-4, 4), rand(-0.18, 0.18), rand(-0.58, -0.06), {
        color: spiritParticleColor(f.color || SPIRIT_COLORS.ghost, 0.22),
        life: rand(180, 420),
        r: rand(0.9, 2.0),
        tail: rand(9, 20),
      });
  ...
  function spiritFollowerCastSource(owner) {
    if (!owner || !owner.spiritFollowers || !owner.spiritFollowers.length) return null;
    let best = owner.spiritFollowers[0], bd = Infinity;
    for (const f of owner.spiritFollowers) {
      const d = Math.hypot(f.x - owner.x, f.y - (owner.y - 64));
      if (d < bd) { bd = d; best = f; }
    }
    return best;
  }
  ...
  function bindSpiritAlly() {
    ...
    spendSpiritFollower(player, p.x, p.y - 48, { count: 16 });
  ...
  function soulFlare() {
    ...
    if ((player.spiritCharges || 0) > 0) spendSpiritFollower(player, tx, ty, { count: 18 });
  ...
  function graveCall() {
    ...
    for (let i = 0; i < chargeCount; i++) {
      ...
      spendSpiritFollower(player, p.x, p.y - 48, { count: 12 });
  ...
  function spawnSmokeZone(x, y, team, opts) {
    ...
    smokeZones.push({
      ...
      y: sourceY,
      team: team || 'hero',
      r: opts.startR || Math.max(26, targetR * 0.22),
      targetR,
      life,
      max: life,
      age: 0,
      tick: 80,
      poison: !!opts.poison,
      color: opts.color || (opts.poison ? '#a7ef5f' : '#8a94a8'),
  ...
  function useEffectAbility(spec, slot, ang) {
    ...
    if (e.kind === 'blackHole') {
      ...
      const source = staffTipOrigin(ang, { type: 'arcaneBloom', t: 0.42, tipPad: 4 });
      spawnBlackHole(p.x, p.y, player.team, gravityAccent(), Object.assign({}, e, { sourceX: source.x, sourceY: source.y }));
      return true;
    }
  ...
  function runLoadoutAbility(spec, slot, ang) {
    ...
    if (spec.use === 'bladecallUltimate') return useBladecallUltimate(ang);
  ...
  //
  // CONTROLS (only while open; P toggles it in lab/debug):
  ...
  function spawnBladeRecallTrail(ax, ay, bx, by, opts) {
    ...
    if (bladeRecallTrails.length > 30) bladeRecallTrails.splice(0, bladeRecallTrails.length - 30);
  }
  ...
  function renderLabPanel() {
    ...
    labPanel.innerHTML = `<div class="sr-labhead"><b>Ability Lab</b><button data-lab-act="collapse" class="sr-labcollapse">Hide</button></div>
      ...
      <div class="sr-labtools">
        ...
        ${pyroTools}
        ${spiritTools}
        <button data-lab-act="pose" class="${poseStudio.active ? 'active' : ''}">Pose Studio</button>
  ...
  function startPoseLab(clsId, buildId) {
    startLab(clsId || 'rogue', buildId || 'acrobat');
    togglePoseStudio(true);
    renderLabPanel();
  }
  ...
  function showMenu() {
    ...
    ov.innerHTML = `<div class="sr-kicker">Wave Fighter</div>
      ...
      <div class="sr-mode-actions">
        <button class="btn alt sr-lab-open" data-lab-cls="${cls.id}">OPEN ABILITY LAB</button>
        <button class="btn alt sr-pose-open" data-act="pose-lab">OPEN POSE STUDIO</button>
      </div>
  ...
  function startFromQuery() {
    if (query.has('poseLab') || query.get('lab') === 'pose') {
      const poseClass = validClassId(query.get('poseClass') || query.get('class') || query.get('cls')) || 'rogue';
      startPoseLab(poseClass, validLabBuildId(poseClass, query.get('build') || query.get('labBuild') || 'acrobat'));
      return;
    }
  ...
  ov.addEventListener('click', e => {
    ...
    if (act === 'pose-lab') { startPoseLab(); return; }
  ...
  function updateGravityDebris(dtStep) {
    ...
    if (player.gravityDebrisJoins && player.gravityDebrisJoins.length) {
      for (let i = player.gravityDebrisJoins.length - 1; i >= 0; i--) {
        const j = player.gravityDebrisJoins[i];
        j.age = (j.age || 0) + (dtStep || STEP);
  ...
  function animate(dt) {
    ...
    if (cls.id === 'mage' && moveAmt < 0.12 && !a.atkActive && Math.random() < dt * 0.000075) {
      ...
      if (gravIdle) {
        ...
        const followers = player.spiritFollowers || [];
        if (followers.length) {
          const s = followers[Math.floor(Math.random() * followers.length)];
          soulParticle(s.x + rand(-8, 8), s.y + rand(-8, 8), rand(-0.12, 0.12), rand(-0.34, -0.02), {
  ...
  function updateGravityDebrisReturn(b, dtStep) {
    ...
    const pull = ease(clamp(b.returnAge / 1900, 0, 1));
    b.vx = b.vx * 0.965 + (dx / d) * (0.10 + pull * 0.34);
    b.vy = b.vy * 0.965 + (dy / d) * (0.10 + pull * 0.34);
    b.angle = (b.angle || 0) + (b.spin || 0.12);
    if (Math.random() < 0.58) {
      const a = Math.atan2(dy, dx) + Math.PI + rand(-0.28, 0.28);
      gravityParticle(b.x + rand(-3, 3), b.y + rand(-3, 3), Math.cos(a) * rand(0.12, 0.52), Math.sin(a) * rand(0.12, 0.52), {
    ...
    if (d < 22 || b.life <= 0) {
      grantGravityDebris(b.x, b.y, { joinDur: 1120 });
      return true;
    }
    return false;
  }
  ...
  function useLiftBoxRock(ang, opts, chosen) {
    ...
    const spd = 14.2 + Math.min(4.2, mass * 1.45);
    projectiles.push({
      kind: 'gravityDebris',
      team: player.team,
      x: cx,
  ...
  function detonateSmokeBomb(b) {
    ...
    radialActorPulse(b.x, cloudY, r, poison ? 10 : 6, b.team, poison ? '#a7ef5f' : '#8a94a8', { poison: poison ? 2400 : 0 });
  ...
  function spawnSpiritBolt(ang) {
    ...
    const mx = origin.x, my = origin.y;
    const follower = spiritFollowerCastSource(player);
    const color = follower && follower.color || SPIRIT_COLORS.ghost;
    if (follower) {
      emitSoulWisp(follower.x, follower.y, mx, my, { count: 10, color, lifeMin: 220, lifeMax: 620, rMin: 0.9, rMax: 2.3 });
      follower.vx += Math.cos(ang) * 0.42;
      follower.vy += Math.sin(ang) * 0.28 - 0.18;
    }
  ...
  function spawnGravityField(x, y, team, color, opts) {
    ...
    gravityFields.push({
      ...
      blackHole,
      sourceX: opts.sourceX, sourceY: opts.sourceY,
      pullPower: opts.force || opts.pullPower || (blackHole ? 1.35 : 1),
      damageTick: blackHole ? BLACK_HOLE_FORM_MS * 0.42 : 0,
    });
    if (blackHole) {
      emitBlackHoleBirth(x, y, r, fieldColor, opts.sourceX != null && opts.sourceY != null ? { x: opts.sourceX, y: opts.sourceY } : null);
  ...
  function blackHoleBirthAmount(g) {
    ...
    return clamp((g.age || 0) / BLACK_HOLE_FORM_MS, 0, 1);
  }
  function blackHoleExpansionAmount(g) {
    const t = blackHoleBirthAmount(g);
    if (t < BLACK_HOLE_EXPAND_START) return 0;
    return ease(clamp((t - BLACK_HOLE_EXPAND_START) / Math.max(0.01, BLACK_HOLE_EXPAND_END - BLACK_HOLE_EXPAND_START), 0, 1));
  }
  function gravityFieldRadius(g) {
    if (!g) return 0;
    if (!g.blackHole) return g.r || 0;
    const birth = blackHoleBirthAmount(g);
    const expand = blackHoleExpansionAmount(g);
    return (g.r || 0) * (0.018 + birth * 0.018 + Math.pow(expand, 0.52) * 0.964);
  }
  ...
  function implodeGravityField(g) {
    ...
    spawnShockwaveRing(g.x, g.y, g.r + (blackHole ? 108 : 70), g.color, {
      ...
      width: blackHole ? 9.2 : g.ultimate ? 6.5 : 5,
      fill: blackHole ? 0.20 : g.ultimate ? 0.14 : 0.09,
  ...
  function updateGravityFields(dtStep) {
    ...
    for (let i = gravityFields.length - 1; i >= 0; i--) {
      ...
      if (g.blackHole && Math.random() < 0.62 + blackHoleBirthAmount(g) * 0.34) {
        const birth = blackHoleBirthAmount(g);
        const expand = blackHoleExpansionAmount(g);
          ...
          for (let k = 0; k < 92; k++) {
            ...
            gravityParticle(g.x + Math.cos(a) * rand(2, 20), g.y + Math.sin(a) * rand(2, 20),
              Math.cos(a) * sp, Math.sin(a) * sp - rand(0.10, 0.55), {
                ...
                shard: true,
                noCore: true,
                dark: Math.random() < 0.28,
              });
          }
          addShake(5.4, 165);
        }
        if (g.sourceX != null && g.sourceY != null && birth < 0.70 && Math.random() < 0.42) {
        ...
        for (let j = 0; j < count; j++) {
          ...
          const tangent = a + Math.PI / 2;
          particles.push({ kind: 'gravity', x: g.x + Math.cos(a) * rr, y: g.y + Math.sin(a) * rr,
  ...
  function drawSpiritOrbit(cx, cy, charges, opts) {
    opts = opts || {};
    ...
    const baseY = opts.worldBaseY == null ? player.y : opts.worldBaseY;
    const followers = ensureSpiritFollowers(player);
    ctx.save();
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    for (let i = 0; i < count; i++) {
      const s = followers[i];
      if (!s) continue;
      const seed = s.phase || i * 2.417;
      const pulse = 0.5 + 0.5 * Math.sin(t * 3.1 + seed);
      const x = s.x - player.x;
      const y = s.y - baseY;
      const r = 5.2 + pulse * 1.9;
      const ang = Math.atan2(s.vy || -0.5, s.vx || -f * 0.5);
      const tail = s.trail && s.trail.length ? s.trail : [{ x: s.x - f * 18, y: s.y + 5 }, { x: s.x, y: s.y }];
      ...
      ctx.lineWidth = 2.2;
      ctx.beginPath();
      for (let j = Math.max(0, tail.length - 5); j < tail.length; j++) {
        const px = tail[j].x - player.x;
        const py = tail[j].y - baseY;
        if (j === Math.max(0, tail.length - 5)) ctx.moveTo(px, py);
        else ctx.lineTo(px, py);
      }
      ctx.stroke();
      ctx.globalCompositeOperation = 'source-over';
      ...
      ctx.beginPath();
      ctx.arc(x - Math.cos(ang) * r * 0.12, y - Math.sin(ang) * r * 0.12, Math.max(0.9, r * 0.20), 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  function drawGravityDebrisOrbit(cx, cy, count, max, opts) {
    opts = opts || {};
    count = clamp(Math.floor(count || 0), 0, max || MAGE_DEBRIS_MAX);
    if (!count) return;
    const t = (player.gravityDebrisSpin || 0) + performance.now() * 0.00018;
    const active = player.anim && player.anim.atkActive && player.anim.visualKind === 'gravityDebris';
    const aim = opts.aim != null ? opts.aim : active ? player.anim.atkAim : aimedAngle();
    const staff = opts.staff || { x: cx + Math.cos(aim) * 58, y: cy + Math.sin(aim) * 42 };
    const castPull = active ? 0.52 + 0.34 * Math.sin(clamp(player.anim.atkT || 0, 0, 1) * Math.PI) : 0.07;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const staffSlot = {
        x: staff.x - Math.cos(aim) * (10 + i * 5) + Math.sin(aim) * (i - 1) * 4,
        y: staff.y - Math.sin(aim) * (8 + i * 4) - Math.cos(aim) * (i - 1) * 4,
      };
      let x = lerp(rawX, staffSlot.x, staffBias);
      let y = lerp(rawY, staffSlot.y, staffBias);
      const join = player.gravityDebrisJoins && player.gravityDebrisJoins.find(j => j.slot === i);
      if (join) {
        const jt = ease(clamp((join.age || 0) / Math.max(1, join.dur || 900), 0, 1));
        const baseY = opts.worldBaseY == null ? player.y : opts.worldBaseY;
        const jx = (join.x - player.x) + Math.sin((join.age || 0) * 0.006 + join.phase) * (1 - jt) * 10;
        const jy = (join.y - baseY) + Math.cos((join.age || 0) * 0.005 + join.phase) * (1 - jt) * 7;
        x = lerp(jx, x, jt);
        y = lerp(jy, y, jt);
      }
      ...
      if (active) {
        ...
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.quadraticCurveTo((x + staff.x) * 0.5 + Math.sin(a) * 10, (y + staff.y) * 0.5 - Math.cos(a) * 8, staff.x, staff.y);
        ctx.stroke();
      }
      ctx.globalCompositeOperation = 'lighter';
  ...
  function solveHeroRagdoll(a, target) {
    ...
    const rawLenCH = Math.hypot(target.headCX - target.shX, target.headCY - target.shY);
    const lenCH = target.flipActive ? clamp(rawLenCH, 13.5, 18.5) : rawLenCH;
    ...
    const KC = target.flipActive ? 0.88 : 0.30, KH = target.flipActive ? 0.82 : 0.26, DMP = target.flipActive ? 0.74 : 0.90;
    ...
    const iterations = target.flipActive ? 5 : 3;
  ...
  function rogueBackflipPose(flip, t) {
    ...
    const curlStrength = flip && flip.curl || 1;
    const tuckStrength = flip && flip.tuck || 1;
    const curl = clamp(kfa(t, clip.curl) * curlStrength, 0, 1.35);
    const tuck = clamp(kfa(t, clip.tuck) * tuckStrength, 0, 1.42);
    const open = clamp(kfa(t, clip.open), 0, 1);
    return {
      spin, travel, curl, tuck, open,
      lead: kfa(t, [[0, 0], [0.18, -0.28], [0.38, 0.24], [0.62, -0.18], [0.88, 0.12], [1, 0]]),
      bodyLean: travel * kfa(t, clip.lean),
      hipLift: kfa(t, clip.hipLift),
      headX: spin * kfa(t, clip.headBack),
      headY: kfa(t, clip.headUp),
      limbWeight: clamp(0.30 + curl * 0.55 + tuck * 0.35, 0, 1),
      foot(legSign, hipY) {
        const side = legSign === -1 ? -1 : 1;
        const forward = kfa(t, clip.footForward);
        const spread = kfa(t, clip.footSpread);
        const flightY = kfa(t, [
          [0, -8 + side * 2],
          [0.14, -27 + side * 3],
          [0.30, hipY - 26 + side * 3],
        ...
        return {
          x: -spin * forward + side * spread,
          y: flightY,
          bend: side === -1 ? -spin : spin,
        };
      },
      hand(side, shX, shY) {
        const forward = kfa(t, clip.armForward);
        const down = kfa(t, clip.armDown);
        const cross = side * (6 - tuck * 3 + open * 5);
        const catchKnee = clamp(tuck * 0.72 + curl * 0.18, 0, 1);
        return {
          x: shX - spin * forward + cross,
          y: shY + down + catchKnee * 4,
        };
      },
    };
  }
  ...
  function drawStick(moveAmt) {
    ...
    const flipPose = flipActive ? rogueBackflipPose(player.flip, flipT) : null;
    const flipCurl = flipPose ? flipPose.curl : 0;
    const flipTuck = flipPose ? flipPose.tuck : 0;
    const flipOpen = flipPose ? flipPose.open : 0;
    const flipLead = flipPose ? flipPose.lead : 0;
    const now = performance.now();
    ...
    if (flipActive) {
      postureLean += flipPose.bodyLean + flipPose.spin * flipLead * 0.10;
      guardCrouch += flipPose.hipLift - 4 * flipTuck;
    }
    ...
    const hipY = -hipH + bob + guardCrouch + breathe + idleY - flipCurl * 3 + clipHipY;
    ...
    if (flipActive) {
      const headW = clamp(0.34 + flipCurl * 0.38 + flipTuck * 0.20, 0, 0.86);
      headCX = lerp(headCX, hipX + flipPose.headX, headW);
      headCY = lerp(headCY, hipY - 34 + flipPose.headY + flipOpen * 3, headW);
    }
    ...
    if (cls.id === 'mage' && player.team === 'hero' && mageSpiritLoadoutActive() && (player.spiritCharges || 0) > 0) {
      drawSpiritOrbit((hipX + shX) * 0.5, (hipY + shY) * 0.5, player.spiritCharges, { worldBaseY: player.y - hoverY });
    }
    ...
    function legFoot(theta, legSign) {
      ...
      if (a.atkActive && a.atkType === 'legSweep') {
        ...
        const fp = flipPose.foot(legSign, hipY);
        const w = flipPose.limbWeight;
    ...
    function rogueFlipHandTarget(side) {
      if (flipPose) return flipPose.hand(side, shX, shY);
      return { x: shX + f * (side > 0 ? 18 : -12), y: shY + 20 };
    }
    ...
    const flipLegScale = flipActive ? lerp(1, 0.64, clamp(flipTuck * 0.84, 0, 1)) : 1;
    const farBend = flipActive ? (lt.bend || flipPose.spin) : -f;
    ...
    // ----- near leg -----
    lt = legFoot(p, -1);
    const nearBend = flipActive ? (lt.bend || -flipPose.spin) : -f;
    ...
    if (attacking && !rogueOff) {
      ...
      if (cls.weapon === 'lance') {
        ...
        if (flipActive) {
          handT = rogueFlipHandTarget(1);
          drawAim = Math.atan2(handT.y - shY, handT.x - shX) + (player.flip.spinDir || -f) * 0.16;
        } else {
          drawAim = f > 0 ? 0.12 : Math.PI - 0.12;
          handT = moveType === 'slide'
            ? { x: shX + f * 46, y: shY + 24 }
  ...
  function drawBlackHoleField(g, t) {
    ...
    const birth = blackHoleBirthAmount(g);
    const expand = blackHoleExpansionAmount(g);
    const seed = ease(clamp(birth / BLACK_HOLE_EXPAND_START, 0, 1));
    const diskAlpha = ease(clamp((expand - 0.08) / 0.92, 0, 1));
    const collapse = ease(clamp((420 - g.life) / 420, 0, 1));
    const phase = g.phase || 0;
    const pulse = 1 + Math.sin(t * 0.010 + phase) * 0.020;
    const rr = gravityFieldRadius(g) * (1 - collapse * 0.18) * pulse;
    const coreR = (2.1 + seed * 2.9 + Math.pow(expand, 0.70) * 38 + Math.sin(t * 0.015 + phase) * (0.35 + expand * 1.7)) * (1 - collapse * 0.28);
    const rimR = coreR + 2.8 + expand * 9 + collapse * 16;
    const diskRot = Math.sin(phase) * 0.13 + Math.sin(t * 0.0009 + phase) * 0.035;
    const diskRx = Math.max(12, rr * (0.54 + expand * 0.16 + collapse * 0.08));
    const diskRy = Math.max(3, rr * (0.065 + expand * 0.12));
    ctx.save();

    function wobblyArc(cx, cy, r, start, span, rough, steps) {
      ctx.beginPath();
      for (let j = 0; j <= steps; j++) {
        const u = j / steps;
        const a = start + span * u;
        const wob = 1 + Math.sin(a * 4.1 + phase + t * 0.0014) * rough + Math.sin(a * 8.3 - phase) * rough * 0.48;
    ...
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    if (g.sourceX != null && g.sourceY != null && birth < 0.78) {
      const fade = 1 - clamp(birth / 0.78, 0, 1);
      const dx = g.x - g.sourceX, dy = g.y - g.sourceY, a = Math.atan2(dy, dx);
      for (let i = 0; i < 3; i++) {
        ...
        ctx.quadraticCurveTo(
          (g.sourceX + g.x) * 0.5 + Math.sin(a) * off + Math.sin(t * 0.005 + i) * 10,
          (g.sourceY + g.y) * 0.5 - Math.cos(a) * off + Math.cos(t * 0.004 + i) * 8,
          g.x + Math.sin(t * 0.006 + i) * 4,
          g.y + Math.cos(t * 0.005 + i) * 4);
        ctx.stroke();
      }
    }
    if (expand < 0.92) {
      const formAlpha = 1 - expand;
      ctx.globalAlpha = (0.18 + seed * 0.28) * formAlpha;
      ctx.strokeStyle = GRAVITY_COLORS.violet;
      ctx.lineWidth = 2.4 + seed * 2.6;
      wobblyArc(g.x, g.y, Math.max(5, 7 + seed * 7 + expand * rr * 0.44), phase + t * 0.002, Math.PI * (1.20 + seed * 0.45), 0.16, 34);
      ctx.stroke();
    }
    ...
    if (collapse > 0.02) {
      ctx.globalAlpha = collapse * 0.58;
      ctx.strokeStyle = GRAVITY_COLORS.violet;
      ctx.lineWidth = 2.2 + collapse * 3.8;
      wobblyArc(g.x, g.y, Math.max(coreR * 1.9, rr * (0.72 - collapse * 0.34)), phase - t * 0.002, Math.PI * 1.76, 0.12, 48);
      ctx.stroke();
    }
    ctx.restore();
  }
  ...
  function drawSmokeZones() {
    ...
    for (const z of smokeZones) {
      ...
      const main = z.poison ? [92, 138, 74] : [74, 82, 100];
      const bright = z.poison ? [176, 242, 91] : [174, 185, 203];
      const dark = z.poison ? [28, 47, 38] : [31, 36, 49];
  ...
  function render(moveAmt) {
    ...
    for (const pt of particles) {
      ...
      if (pt.kind === 'flame') {
        ...
        ctx.globalCompositeOperation = 'lighter';
        ctx.lineCap = 'round';
        if (pt.shard || pt.noCore) {
          const tail = (pt.tail || 14) * (0.45 + fade * 0.70);
          const side = Math.max(0.7, rr * 0.75);
          ctx.globalAlpha = (pt.dark ? 0.34 : 0.46) * fade;
          ctx.strokeStyle = pt.dark ? 'rgba(5,3,13,0.95)' : pt.color || '#8f7dff';
          ctx.lineWidth = Math.max(0.75, rr * (pt.dark ? 1.25 : 0.72));
          ctx.beginPath();
          ctx.moveTo(sx - Math.cos(ang) * tail, sy - Math.sin(ang) * tail);
          ctx.quadraticCurveTo(
            sx - Math.cos(ang) * tail * 0.32 - Math.sin(ang) * side,
            sy - Math.sin(ang) * tail * 0.32 + Math.cos(ang) * side,
            sx,
            sy
          );
          ctx.stroke();
          ctx.globalAlpha = (pt.dark ? 0.58 : 0.86) * fade;
          ctx.fillStyle = pt.dark ? GRAVITY_COLORS.void : pt.color || '#8f7dff';
          ctx.beginPath();
          ctx.moveTo(sx + Math.cos(ang) * rr * 2.0, sy + Math.sin(ang) * rr * 2.0);
          ctx.lineTo(sx + Math.cos(ang + 2.45) * rr * 0.72, sy + Math.sin(ang + 2.45) * rr * 0.72);
          ctx.lineTo(sx - Math.cos(ang) * rr * 1.8, sy - Math.sin(ang) * rr * 1.8);
          ctx.lineTo(sx + Math.cos(ang - 2.45) * rr * 0.72, sy + Math.sin(ang - 2.45) * rr * 0.72);
          ctx.closePath();
          ctx.fill();
    ...
    for (const b of projectiles) {
      if (b.kind === 'dagger') {
        ...
        ctx.fillStyle = b.poison ? '#2f432f' : '#3d4352';
        ctx.strokeStyle = b.poison ? '#a7ef5f' : '#aeb9cb';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ...
        ctx.strokeStyle = b.poison ? '#e0ff87' : '#dce5f2';
        ctx.beginPath();
        ...
        ctx.fillStyle = SPIRIT_COLORS.pale;
        ctx.beginPath();
        ctx.arc(b.x + Math.cos(ang) * r * 0.12, b.y + Math.sin(ang) * r * 0.12, Math.max(1.0, r * 0.24), 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
      } else {
        const r = b.r || 10;
      ...
      if (freeze > 0) { freeze -= dt; }     // hit-stop: pause sim, hold the frame
        ...
        for (let i = projectiles.length - 1; i >= 0; i--) {
          ...
          if (b.kind === 'dagger') {
            ...
            b.vy += b.poison ? 0.255 : 0.238;
            b.vx *= 0.998;
            b.angle = Math.atan2(b.vy, b.vx);
            b.spinAngle = (b.spinAngle || 0) + (b.spin || 0.18);
            const trailDensity = b.poison ? 0.82 : 0.68;
          ...
          if ((crate || hitPlatform) && b.bounce > 0 && !struckActor) {
            ...
            else { b.vx *= -0.72; b.vy *= 0.84; }
            b.x = px; b.y = py;
            b.angle = Math.atan2(b.vy, b.vx);
            burst(b.x, b.y, b.color, b.kind === 'smokeBomb' ? 5 : 8, b.kind === 'smokeBomb' ? 1.4 : 2.6);
            continue;
          }
  ...
  function exposeDebugApi() {
    ...
    if (exposed) {
      ...
      if (root && root.dataset) {
        ...
        root.dataset.stickPoseStudio = poseStudio.active ? '1' : '0';
      ...
      if (root && root.dataset) {
        ...
        delete root.dataset.stickPoseStudio;
