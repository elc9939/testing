...
describe('Mini Hub usability control gates', () => {
  ...
  it('guards Macro Lab and Passive task side-effect controls while state is unknown', async () => {
    ...
    expect(macro).toContain('Connect Macro Lab to inspect saved definitions and run history.');
    expect(macro).toContain('Loading macro definitions from Macro Lab.');
    expect(macro).toContain('Loading action catalog.');
    expect(macro).toContain('Loading recent Macro Lab runs.');
    ...
    expect(passive).toContain("if (!settings) return serviceError ? 'Offline' : 'Not checked'");
    expect(passive).toContain("return snapshot ? 'No backup health' : serviceError ? 'Offline' : 'Not loaded'");
    expect(passive).toContain("if (loading && !snapshot) return 'checking'");
    expect(passive).toContain("if (!snapshot) return 'not loaded'");
    expect(passive).toContain("if (!snapshot) return serviceError ? 'Offline' : 'Not checked'");
    expect(passive).toContain("if (typeof value !== 'number' || !Number.isFinite(value)) return 'not measured'");
    expect(passive).toContain('Loading passive task outputs.');
