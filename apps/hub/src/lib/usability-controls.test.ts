...
describe('Mini Hub usability control gates', () => {
  ...
  it('collapses Research service failures and disables AI OS-backed research actions', async () => {
    ...
    expect(source).toContain("type ResearchEffort = 'quick' | 'standard' | 'deep'");
    expect(source).toContain('const effortOptions: ResearchEffortOption[]');
    expect(source).toContain('function selectEffort');
    expect(source).toContain('function effortFromResearchMode');
    expect(source).toContain('Run Research');
    expect(source).toContain('Ask once, get a readable brief');
    expect(source).toContain('Readable research brief');
    expect(source).toContain('Quick Report');
    expect(source).toContain('function reportParagraphs');
    expect(source).toContain('function conciseList');
    expect(source).toContain('function topResearchSources');
    expect(source).toContain('function displayResearchTitle');
    expect(source).toContain('function displayRunTitle');
    expect(source).toContain('function displayMonitorName');
    expect(source).toContain('function addRunSourcesAsSeeds');
    expect(source).toContain('function useFollowUpSuggestion');
    expect(source).toContain('Use Sources Next');
    expect(source).toContain("href={hubHref('/activity')}");
    expect(source).toContain("conciseList(selectedRun.report.reliability_notes, 'reliability'");
    ...
    expect(source).toContain('Diagnostics and raw run data');
    expect(source).toContain('Raw extracted sources');
    ...
    expect(source).toContain('function researchEffortTitle');
    ...
    expect(source).toContain('title={researchEffortTitle(item)}');
  ...
  it('blocks AI OS service-backed work until a status snapshot is loaded', async () => {
    ...
    expect(source).toContain('readCachedAiOsDashboardSnapshot');
    expect(source).toContain('writeAiOsDashboardCache');
    expect(source).toContain('function hydrateAiOsCache');
    expect(source).toContain('function currentAiOsDashboardSnapshot');
    expect(source).toContain('function syncAiOsDashboardSnapshot');
    expect(source).toContain('Showing saved AI OS status while the desktop service refreshes.');
    expect(source).toContain('actions wait for a live reconnect');
    expect(source).toContain('startupRetryCount > 20 || liveStatusReady');
    expect(source).toContain('aiOsReady = Boolean(status && liveStatusReady)');
    ...
    expect(source).toContain('aiOsRefreshing = loading || backgroundRefreshing');
    expect(source).toContain('aiOsDefaultRefreshTitle = aiOsRefreshTitle(aiOsRefreshing)');
    expect(source).toContain("aiOsCommandRefreshTitle = aiOsRefreshTitle(aiOsRefreshing, 'Refresh AI OS status before running command actions.')");
    expect(source).toContain("aiOsProfileRefreshTitle = aiOsRefreshTitle(aiOsRefreshing, 'Refresh AI OS status and machine profile.')");
    expect(source).toContain("aiOsAdvancedCommandRefreshTitle = aiOsRefreshTitle(aiOsRefreshing, 'Refresh AI OS status before using advanced command controls.')");
    ...
    expect(source).toContain('disabled={aiOsRefreshing}');
    expect(source).toContain("{aiOsRefreshing ? 'Refreshing' : 'Refresh status'}");
  ...
  it('keeps Career and Study inline edits read-only when save capability drops', async () => {
    ...
    expect(career).toContain('interface ApplyQueueItem');
    expect(career).toContain('topApplyQueue = applyQueue.slice(0, 6).map(applyQueueItem)');
    expect(career).toContain('function applyQueueScore');
    expect(career).toContain('function compareApplyQueueJobs');
    expect(career).toContain('function applicationAngle');
    expect(career).toContain('Career apply queue');
    expect(career).toContain('ranked by fit, timing, source, and urgency');
    expect(career).toContain('No active leads are ready for the apply queue.');
    ...
    expect(career).toContain("reviewDiscoveredLead(job, 'watch')");
    expect(career).toContain("reviewDiscoveredLead(job, 'archive')");
    expect(career).toContain('Discovered by Career Discovery');
    expect(career).toContain('Discovered Leads');
    expect(career).toContain('ranked by fit');
    expect(career).toContain('Career Discovery review:');
    expect(career).toContain('interface CareerDiscoveryProfile');
    ...
    expect(career).toContain('interface CareerDiscoveryReadiness');
    expect(career).toContain('function buildCareerDiscoveryLearningSummary');
    expect(career).toContain('function buildCareerDiscoveryReadiness');
    expect(career).toContain('function maxScoutCareerDiscoveryProfile');
    expect(career).toContain('function enableCareerDiscoveryMode');
    expect(career).toContain('function enableMaxScout');
    expect(career).toContain('function enableMaxPowerSearch');
    expect(career).toContain('function runCareerDiscoveryNow');
    expect(career).toContain('patchPassiveSettings');
    expect(career).toContain('Career Discovery is not configured');
    expect(career).toContain('Career Discovery needs service');
    expect(career).toContain('Enable Max Scout');
    expect(career).toContain('Max Power Search');
    expect(career).toContain('maxPowerSearch');
    expect(career).toContain('careerDiscoveryMaxPowerSearch');
    expect(career).toContain('Max power continuous');
    expect(career).toContain('Keep Max Power Search on');
    expect(career).toContain('Graduation guard: B.S. May 2026');
    expect(career).toContain('Run Discovery Now');
    expect(career).toContain('No saved Max Scout profile exists, so passive runs only watch existing saved-job domains.');
    expect(career).toContain('careerDiscoveryActiveSourceLaneCount');
    ...
    expect(career).toContain('priorityCompanies');
    ...
    expect(career).toContain('wrong graduation year');
    expect(career).toContain('wrong start date');
    expect(career).toContain('qualification mismatch');
    ...
    expect(career).toContain('Saved Career Discovery filters for passive role research.');
    expect(career).toContain('clientData.saveCareerAction');
    expect(career).toContain('clientData.updateCareerAction');
    expect(career).toContain('clientData.saveSettings');
    ...
    expect(career).toContain('title={careerDiscoveryProfileTitle(careerControlState)}');
    ...
    expect(career).toContain('Use this profile for routine passive role research');
    expect(career).toContain('Saved-role seeds:');
    expect(career).toContain('Duplicate guard:');
  ...
  it('keeps the hydrated smoke script dependency-free and focused on route recovery', async () => {
    ...
    expect(source).toContain("safeActionLabels: ['Connect AI OS', 'Run Research', 'Retry Service']");
    expect(source).toContain('safeActionFallbacks');
    expect(source).toContain("'Run Research': ['Connect AI OS', 'Checking AI OS', 'Setup', 'Checking saved research monitors']");
  ...
  it('guards Settings sync, export, and endpoint actions with readable busy states', async () => {
    ...
    expect(source).toContain('Tunnel watcher and startup readiness');
