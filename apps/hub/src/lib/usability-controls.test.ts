...
describe('Mini Hub usability control gates', () => {
  ...
  it('blocks AI OS service-backed work until a status snapshot is loaded', async () => {
    ...
    expect(source).toContain('AI OS diagnostic is compacted for display. Retry or open Settings Feature Wiring for service setup.');
    ...
    expect(source).toContain('aiOsCommandButtonTitle = aiOsActionBlocked');
    expect(source).toContain('aiOsAutotuneButtonTitle = aiOsActionBlocked');
    ...
    expect(source).toContain('title={foundationBackupButtonTitle}');
  ...
  it('guards Macro Lab and Passive task side-effect controls while state is unknown', async () => {
    ...
    expect(macro).toContain("macroPanicTitle = macroActionTitleForState(macroControlTitle, 'panic', selectedMacro)");
    expect(macro).toContain("macroToggleArmedTitle = macroActionTitleForState(macroControlTitle, 'toggle-armed', selectedMacro)");
    expect(macro).toContain("macroDryRunTitle = macroActionTitleForState(macroControlTitle, 'dry-run', selectedMacro)");
    expect(macro).toContain("macroRunConfirmedTitle = macroActionTitleForState(macroControlTitle, 'run-confirmed', selectedMacro)");
    expect(macro).toContain("macroRecordTitle = macroActionTitleForState(macroControlTitle, 'record', selectedMacro)");
    expect(macro).toContain('title={macroPanicTitle}');
    expect(macro).toContain('title={macroToggleArmedTitle}');
    expect(macro).toContain('title={macroDryRunTitle}');
    expect(macro).toContain('title={macroRunConfirmedTitle}');
    expect(macro).toContain('title={macroRecordTitle}');
    ...
    expect(passive).toContain('passiveInitialLoading = loading && !snapshot');
    expect(passive).toContain('passiveWriteTitle = passiveDisabledReason({ loading: passiveInitialLoading, busyId, serviceError, serviceReady: passiveServiceReady })');
    ...
    expect(passive).toContain('passiveRefreshBlockedReason = passiveRefreshDisabledReason({ loading: loading || backgroundRefreshing, busyId })');
    expect(passive).toContain('readCachedPassiveSnapshot');
    expect(passive).toContain('writePassiveSnapshotCache');
    expect(passive).toContain('function hydratePassiveCache');
    expect(passive).toContain('Showing saved snapshot while live Passive Tasks refreshes.');
    expect(passive).toContain('Saved snapshot {displayShortDate(cachedAt)}.');
    ...
    expect(passive).toContain('Passive Tasks diagnostic is compacted for display. Retry Refresh or check Feature Wiring.');
    expect(passive).toContain('Passive source diagnostic is compacted for display. Refresh source health or check Feature Wiring.');
    expect(passive).toContain('Passive trigger diagnostic is compacted for display. Refresh triggers or check Feature Wiring.');
    expect(passive).toContain('Passive worker diagnostic is compacted for display. Retry Refresh or check Feature Wiring.');
    expect(passive).toContain('Passive run diagnostic is compacted for display. Open logs or retry the task.');
    ...
    expect(passive).toContain("if (!settingsValue) return 'Refresh needed'");
    expect(passive).toContain("return snapshotValue ? 'Refresh backup check' : 'Refresh needed'");
    ...
    expect(passive).toContain("if (!snapshotValue) return 'Refresh needed'");
    expect(passive).toContain("if (!workerValue?.startedAt) return 'Waiting for startup'");
    ...
    expect(passive).toContain('{passiveDigestEmptyMessage(snapshot, loading, serviceError, summarizedDigestCards.length)}');
    ...
    expect(passive).not.toContain('Loading passive task outputs...');
    expect(passive).toContain('<strong>{passiveEngineLabel(snapshot, settings, loading)}</strong>');
    ...
    expect(passive).toContain('<details class="advanced-passive-settings">');
    expect(passive).toContain('<span>Advanced options</span>');
    expect(passive).toContain('Idle guardrails, AI fallback, family switches, and watched scopes');
    ...
    expect(passive).toContain("{loading || backgroundRefreshing ? 'Refreshing' : 'Refresh'}");
  ...
  it('keeps Productivity Google writes guarded while cached data remains inspectable', async () => {
    ...
    expect(source).toContain('googleConnectionManageDisabled = loading || !canAct || Boolean(actionBusyKey)');
    ...
    expect(source).not.toContain('on:click={connectGoogle}');
    ...
    expect(source).toContain('title={gmailThreadReadActionTitle(productivityControlTitleState, thread)}');
    expect(source).toContain('title={gmailThreadImportantActionTitle(productivityControlTitleState, thread)}');
    expect(source).toContain('title={gmailThreadArchiveActionTitle(productivityControlTitleState, thread)}');
    expect(source).toContain('title={gmailThreadReadActionTitle(productivityControlTitleState, selectedGmailThread)}');
    expect(source).toContain('title={gmailThreadImportantActionTitle(productivityControlTitleState, selectedGmailThread)}');
    expect(source).toContain('title={gmailThreadArchiveActionTitle(productivityControlTitleState, selectedGmailThread)}');
  ...
  it('keeps Today recommendation actions tied to live capability readiness', async () => {
    ...
    expect(source).toContain('$: priorityEmptyText');
    expect(source).toContain('Priority Queue needs a refresh. Refresh Today or open Settings Feature Wiring.');
    ...
    expect(source).toContain('$: systemEmptyText');
  ...
  it('shows Analytics refresh as a guarded async action with readable failures', async () => {
    ...
    expect(source).toContain('visibleViewMessage = viewState ===');
    ...
    expect(source).toContain('Raw Analytics cache error:');
    ...
    expect(source).toContain("if ($clientData.status === 'error') return compactAnalyticsIssue($clientData.error || 'Cache status reported an error.')");
    ...
    expect(source).toContain("if (viewState === 'error') return compactAnalyticsIssue($clientData.error || 'The browser cache needs attention before analytics can render.')");
    ...
    expect(source).toContain("if (viewState === 'error') return compactAnalyticsIssue($clientData.error || 'The browser cache needs attention before study trends can render.')");
  ...
  it('guards Settings sync, export, and endpoint actions with readable busy states', async () => {
    ...
    expect(source).toContain('Passive Tasks diagnostic is compacted for display. Retry Passive or check Feature Wiring.');
    expect(source).toContain('Passive restore point diagnostic is compacted for display. Retry Passive or check Data & Recovery.');
    expect(source).toContain('Capability Registry diagnostic is compacted for display. Retry Refresh Capabilities or check Feature Wiring.');
    expect(source).toContain('Settings/cache diagnostic is compacted for display. Retry Sync Now or check Feature Wiring.');
    expect(source).toContain('Desktop Services diagnostic is compacted for display. Save service URLs, then run Check Services.');
    ...
    expect(source).toContain('Action Ledger diagnostic is compacted for display. Retry Refresh or check Feature Wiring.');
    expect(source).toContain('Action Ledger source diagnostic is compacted for display. Retry Refresh or check Feature Wiring.');
